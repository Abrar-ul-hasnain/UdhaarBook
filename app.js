...
function saveUdhaar(data) {
  ...
  if (window.firebaseSaveUdhaar) window.firebaseSaveUdhaar(data);
}
...
function updateUdhaar(id, changes) {
  ...
  if (window.firebaseUpdateUdhaar) window.firebaseUpdateUdhaar(id, changes);
}
...
function generateContractId() {
  ...
  const num  = String(getAllUdhaars().length + 1).padStart(3, '0');
  return `UB-${year}-${num}`;
}
...
function renderHomeScreen() {
  ...
  const total    = given.reduce((sum, u) => sum + u.amount, 0);
 
  document.getElementById('stat-given').textContent   = formatAmount(total);
...
function buildConfirmationLink(id) {
  ...
  if (!u) return '';
  const shortData = { i: u.id, b: u.borrower, l: u.lender, a: u.amount, d: u.dueDate, p: u.phone, n: u.note || '' };
  return `https://udhaarbook.netlify.app/?confirm=${id}&data=${encodeURIComponent(JSON.stringify(shortData))}`;
}
...
function loadSendConfirmationScreen(id) {
  ...
  document.getElementById('sc-wa-btn').onclick = () => {
    window.open(buildWhatsAppLink(u.phone, message), '_blank');
  };
 
  document.getElementById('sc-skip-link').onclick = (e) => {
    e.preventDefault();
    loadContractScreen(u.id);
  };
 
  if (window.firebaseListenUdhaar) {
    window.firebaseListenUdhaar(u.id, function(updated) {
      if (updated.status === 'active') {
        Swal.fire({
          icon:               'success',
          title:              'Confirmed!',
          text:               `${updated.borrower} has confirmed the udhaar. Contract is now active.`,
...
function cancelUdhaar(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      if (window.firebaseDeleteUdhaar) {
        window.firebaseDeleteUdhaar(id);
      }
...
function showThankYouScreen() {
  document.getElementById('app').innerHTML = `
    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;padding:32px;text-align:center;font-family:Inter,sans-serif;background:white;">
      <div style="width:72px;height:72px;background:#E1F5EE;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:32px;margin:0 auto 20px;">✓</div>
      <h2 style="font-size:20px;font-weight:600;color:#0D1F1A;margin-bottom:10px;">Udhaar Confirmed</h2>
      <p style="font-size:14px;color:#4A6560;line-height:1.7;max-width:280px;">You have successfully acknowledged this udhaar. Both parties now have a mutual digital record. Please ensure payment is made by the due date.</p>
      <p style="font-size:12px;color:#8A9E99;margin-top:24px;">You may now close this tab.</p>
    </div>`;
}
 
function showDeclinedScreen() {
  document.getElementById('app').innerHTML = `
    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;padding:32px;text-align:center;font-family:Inter,sans-serif;background:white;">
      <div style="width:72px;height:72px;background:#FDECEA;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:32px;margin:0 auto 20px;">✕</div>
      <h2 style="font-size:20px;font-weight:600;color:#0D1F1A;margin-bottom:10px;">Udhaar Declined</h2>
      <p style="font-size:14px;color:#4A6560;line-height:1.7;max-width:280px;">You have declined this udhaar request. The lender will be informed accordingly.</p>
      <p style="font-size:12px;color:#8A9E99;margin-top:24px;">You may now close this tab.</p>
    </div>`;
}
...
function markPaid(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      updateUdhaar(id, { status: 'paid', paidDate: dayjs().format('YYYY-MM-DD') });
      const u = getUdhaarById(id);
      renderContractStamp(u);
      ...
      Swal.fire({
        ...
        text:               'The contract has been successfully closed.',
        confirmButtonColor: '#0F6E56'
      });
    }
  });
...
function renderDashboard(filter = 'all') {
  const all      = getAllUdhaars();
  const filtered = filter === 'all' ? all : all.filter(u => getBadgeStatus(u) === filter);
...
function exportCSV() {
  ...
  if (all.length === 0) {
    Swal.fire({ icon: 'info', title: 'No Data', text: 'No udhaar records to export.', confirmButtonColor: '#0F6E56' });
    return;
  }
 
  const headers = ['ID', 'Borrower', 'Phone', 'Amount (PKR)', 'Created Date', 'Due Date', 'Status', 'Reminders Sent', 'Note'];
  const rows    = all.map(u => [
    u.id, u.borrower, u.phone, u.amount,
    formatDate(u.createdDate), formatDate(u.dueDate),
    u.status, u.remindersCount, u.note || '-'
  ]);
  ...
  Swal.fire({
    icon:               'success',
    title:              'Exported!',
    text:               `${all.length} records downloaded successfully.`,
    confirmButtonColor: '#0F6E56'
  });
}
...
function changeTone(tone) {
  ...
  const u = getUdhaarById(currentReminderId);
  if (!u) return;
...
function checkUpcomingDues() {
  const upcoming = getAllUdhaars().filter(u => {
    const days   = getDaysRemaining(u.dueDate);
    const status = getBadgeStatus(u);
    return (status === 'active' || status === 'soon') && days >= 0 && days <= 3;
  });
  if (upcoming.length > 0) showDueReminder(upcoming, 0);
}
...
function showDueReminder(list, index) {
  ...
  Swal.fire({
    ...
    html:               `<p style="font-size:14px;color:#4A6560;line-height:1.7;"><strong>${u.borrower}</strong> owes you <strong>${formatAmount(u.amount)}</strong><br>Payment is due <strong>${daysText}</strong> (${formatDate(u.dueDate)})</p>`,
    showCancelButton:   true,
    confirmButtonColor: '#0F6E56',
...
style.textContent = `@keyframes gooeyRipple {
